...
glm::vec3 CPURenderer::cosPalette(float t, const glm::vec3& a, const glm::vec3& b) {
    float f = 0.5f + 0.5f * std::cos(glm::pi<float>() * std::clamp(t, 0.0f, 1.0f));
    return glm::mix(b, a, f);
}

// Background: vertical gradient + glow emitted toward nearby fractal surface
glm::vec3 CPURenderer::background(const glm::vec3& dir, float minD, const RenderParams& p) {
...
// trap: orbit trap value for coloring; minD: minimum SDF value seen (drives glow)
float CPURenderer::march(const Ray& ray, const RenderParams& p, float& trap, float& minD) {
    float t  = 0.001f;
    trap = 1e10f;
    minD = 1e10f;
    ...
    for (int i = 0; i < p.maxSteps; ++i) {
        ...
        if (d < p.epsilon) return t;           // hit
        if (t > p.maxDist) break;              // escaped
        t += std::max(d, p.epsilon * 0.5f);   // min step avoids infinite loop near interior
    }
    return -1.0f; // miss
}
...
glm::vec3 CPURenderer::calcNormal(const glm::vec3& pos, const RenderParams& p) {
    ...
    return glm::normalize(glm::vec3(
        SDF::evaluate(pos + glm::vec3(e,0,0), p, dummy) - SDF::evaluate(pos - glm::vec3(e,0,0), p, dummy),
        SDF::evaluate(pos + glm::vec3(0,e,0), p, dummy) - SDF::evaluate(pos - glm::vec3(0,e,0), p, dummy),
        SDF::evaluate(pos + glm::vec3(0,0,e), p, dummy) - SDF::evaluate(pos - glm::vec3(0,0,e), p, dummy)
    ));
}

// Penumbra soft shadow: march toward light, accumulate proximity to surfaces
float CPURenderer::softShadow(const glm::vec3& pos, const glm::vec3& ldir, const RenderParams& p) {
...
float CPURenderer::ambientOcc(const glm::vec3& pos, const glm::vec3& nor, const RenderParams& p) {
    if (!p.light.aoEnabled) return 1.0f;
    float occ   = 0.0f;
    float scale = 1.0f;
...
glm::vec3 CPURenderer::shadePixel(int px, int py, const RenderParams& p) {
    ...
    float spec = std::pow(std::max(glm::dot(nor, halfVec), 0.0f), p.light.shininess);
    ...
    glm::vec3 diffuse  = p.light.diffuse * diff * shadow * p.light.color * surfCol;
