...
private:
    ...
    float     march        (const Ray& ray, const RenderParams& p, float& trap, float& minD);
    glm::vec3 calcNormal   (const glm::vec3& pos, const RenderParams& p);
    float     softShadow   (const glm::vec3& pos, const glm::vec3& ldir, const RenderParams& p);
    float     ambientOcc   (const glm::vec3& pos, const glm::vec3& nor,  const RenderParams& p);
    glm::vec3 cosPalette   (float t, const glm::vec3& a, const glm::vec3& b);
    glm::vec3 background   (const glm::vec3& dir, float minD, const RenderParams& p);
};
