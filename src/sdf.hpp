...
// ── Mandelbulb ────────────────────────────────────────────────────────────────
// Classic power-N spherical Mandelbulb. Power 8 gives the canonical shape.
inline float mandelbulb(glm::vec3 pos, const MandelbulbParams& p, float& trap) {
    ...
    for (int i = 0; i < p.iterations; ++i) {
        ...
        if (r < 1e-8f) { trap = 0.0f; return 0.0f; }
    ...
    if (r < 1e-8f || dr < 1e-8f) return 0.0f;
    return 0.5f * std::log(r) * r / dr;
}

// ── Mandelbox ─────────────────────────────────────────────────────────────────
// Box + sphere fold iterated system. Scale drives the overall shape dramatically.
...
// 3D cross-section of the 4D quaternion Julia set f(z) = z^2 + c.
// Slice at w=0. Different values of c give wildly different shapes.
inline float julia(glm::vec3 pos, const JuliaParams& p, float& trap) {
    ...
    for (int i = 0; i < p.iterations; ++i) {
        float r2 = glm::dot(z, z);
        if (r2 < 1e-10f) { trap = 0.0f; return 0.0f; }

        trap = std::min(trap, r2);

        if (r2 > 4.0f) {
            float r = std::sqrt(r2);
            if (dz < 1e-10f) return 0.0f;
...
// ── Dispatch ──────────────────────────────────────────────────────────────────
inline float evaluate(const glm::vec3& pos, const RenderParams& p, float& trap) {
    switch (p.fractalType) {
        case FractalType::Mandelbulb: return mandelbulb(pos, p.mandelbulb, trap);
        case FractalType::Mandelbox:  return mandelbox (pos, p.mandelbox,  trap);
        case FractalType::Julia:      return julia     (pos, p.julia,      trap);
    }
    return 1.0f;
}
